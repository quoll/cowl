(ns cowl.io-test
  (:require [clojure.test :refer [deftest testing is]]
            ...
            [cowl.impl.data-prop :refer [data-prop-domain]]
            [cowl.io.functional]
            [cowl.io.iop :refer [emit]])
  (:import [java.io StringWriter]))

(defn st
  "Creates a StringWriter, emits the object to it, and returns the output as a string."
  [obj]
  (let [s (StringWriter.)]
    (emit obj s)
    (str s)))
...
(deftest data-prop-domain-test
  (testing "DataPropertyDomain with simple keywords"
    (is (= "DataPropertyDomain(:hasAge :Person)\n"
           (st (data-prop-domain :hasAge :Person))))))
